...
void main() {
  group('BaseResponse Tests', () {
    ...
    group('toJson Tests (Manual Converters)', () {
      test('should serialize BaseResponse with String data', () {
        final response = BaseResponse<String>(
          success: true,
          message: 'Success',
          data: 'Hello World',
        ...
        expect(json['data'], equals('Hello World'));
        expect(json['code'], equals(200));
      });

      test('should serialize BaseResponse with User data', () {
        final user = User(id: 1, name: 'John Doe');
        final response = BaseResponse<User>(
          success: true,
        ...
        final json = response.toJson((value) => value.toJson());
        
        expect(json['success'], equals(true));
        expect(json['message'], equals('User retrieved'));
        expect(json['data'], isA<Map<String, dynamic>>());
        expect(json['data']['id'], equals(1));
        expect(json['data']['name'], equals('John Doe'));
        expect(json['code'], equals(200));
      });

      test('should serialize BaseResponse with List<Post> data', () {
        final posts = [
          Post(id: 1, title: 'First Post', content: 'Content 1'),
          Post(id: 2, title: 'Second Post', content: 'Content 2'),
        ];
        ...
        final json = response.toJson((value) => 
          value.map((post) => post.toJson()).toList()
        );
        
        expect(json['success'], equals(true));
        expect(json['message'], equals('Posts retrieved'));
        expect(json['data'], isA<List>());
        expect((json['data'] as List).length, equals(2));
        expect(json['code'], equals(200));
      });

      test('should serialize BaseResponse with null data', () {
        final response = BaseResponse<User?>(
          success: false,
        ...
        expect(json['message'], equals('User not found'));
        expect(json['data'], isNull);
        expect(json['code'], equals(404));
      });
    });

    group('DataConverter Tests', () {
      test('should handle toJson operation correctly', () {
        const converter = DataConverter<User>();
        final user = User(id: 1, name: 'Test User');
        
        final result = converter.toJson(user);
        expect(result, equals(user));
      });

      test('should handle toJson with null correctly', () {
        const converter = DataConverter<User>();
      ...
      test('should handle primitive data types', () {
        const stringConverter = DataConverter<String>();
        const intConverter = DataConverter<int>();
        const boolConverter = DataConverter<bool>();
        
        expect(stringConverter.toJson('test'), equals('test'));
        expect(intConverter.toJson(42), equals(42));
        expect(boolConverter.toJson(true), equals(true));
      });
    });
    ...
    group('Manual JSON Serialization Tests', () {
      test('should serialize and deserialize with String data', () {
        final response = BaseResponse<String>(
          success: true,
          message: 'Success',
          data: 'Hello World',
        ...
        final deserializedResponse = BaseResponse<String>.fromJson(
          json,
          (json) => json as String,
        );

        expect(deserializedResponse.success, equals(true));
        expect(deserializedResponse.message, equals('Success'));
        expect(deserializedResponse.data, equals('Hello World'));
        expect(deserializedResponse.code, equals(200));
      });

      test('should handle User data with manual serialization', () {
        final user = User(id: 1, name: 'John Doe');
        final response = BaseResponse<User>(
          success: true,
          message: 'User retrieved',
        ...
        final deserializedResponse = BaseResponse<User>.fromJson(
          json,
          (json) => User.fromJson(json as Map<String, dynamic>),
        );

        expect(deserializedResponse.success, equals(true));
        expect(deserializedResponse.message, equals('User retrieved'));
        expect(deserializedResponse.data?.id, equals(1));
        expect(deserializedResponse.data?.name, equals('John Doe'));
        expect(deserializedResponse.code, equals(200));
      });

      test('should handle null data properly', () {
        final response = BaseResponse<User?>(
          success: false,
          message: 'User not found',
          data: null,
        ...
        expect(deserializedResponse.success, equals(false));
        expect(deserializedResponse.message, equals('User not found'));
        expect(deserializedResponse.data, isNull);
        expect(deserializedResponse.code, equals(404));
      });
    });
